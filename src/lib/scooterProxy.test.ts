...
const env = { SCOOTER_SNAPSHOT_API_URL: 'https://cache.example',
...
describe('edge snapshot proxy', () => {
  ...
  it('uses one cache request and preserves freshness headers', async () => {
    const fetcher = vi.spyOn(globalThis, 'fetch').mockResolvedValue(Response.json({ vehicles: [] }, { headers: { 'Cache-Control': 'public, max-age=30' } }));
